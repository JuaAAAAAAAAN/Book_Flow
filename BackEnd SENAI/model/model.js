...
const userModel = {
    ...
    validate: async (email, senha) => {
        try {
            const [dadosUsuario] = await connection.query("SELECT * FROM cadastro WHERE email=?", [email]);
    ...
    registerBookflow: async (id, nome, idade, email, senha) => {
        ...
        const [result] = await connection.query("INSERT INTO cadastro values(?,?,?,?,?)", [id, nome, idade, email, hashPassword])
            .catch(erro => console.log(erro));
        return result
    },
